...
-(CGSize)collectionView:(UICollectionView *)collectionView layout:(UICollectionViewLayout *)collectionViewLayout sizeForItemAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (_itemWidth == 0) {
        ...
        CGFloat width = collectionView.layoutM.width;
        if (width <= 0) {
            width = collectionView.frame.size.width;
        }
        _itemWidth = (width - flowLayout.minimumInteritemSpacing * (_itemCounts - 1))/_itemCounts;
    }
    if (height > 0) {
        return CGSizeMake(_itemWidth, height);
    }
    UICollectionViewCell *cell = self.renderBlock(_collectionViewCell, indexPath, object);
...
-(UICollectionReusableView *)collectionView:(UICollectionView *)collectionView viewForSupplementaryElementOfKind:(NSString *)kind atIndexPath:(NSIndexPath *)indexPath{
    ...
    if (kind == UICollectionElementKindSectionHeader) {
        if (self.headerViewBlock) {
            resuableView = self.headerViewBlock(resuableView, indexPath.section, object);
            CGFloat width = collectionView.layoutM.width;
            if (width <= 0) {
                width = collectionView.frame.size.width;
            }
            resuableView.layoutM.width = width;
            [resuableView.layoutM applyLayoutPreservingOrigin:NO];
        }
        
    }else if (kind == UICollectionElementKindSectionFooter){
        if (self.footerViewBlock) {
            resuableView = self.footerViewBlock(resuableView, indexPath.section, object);
            CGFloat width = collectionView.layoutM.width;
            if (width <= 0) {
                width = collectionView.frame.size.width;
            }
...
-(CGSize)collectionView:(UICollectionView *)collectionView layout:(UICollectionViewLayout *)collectionViewLayout referenceSizeForHeaderInSection:(NSInteger)section{
    ...
    reusableView.layoutM.width = width;
...
-(UIEdgeInsets)collectionView:(UICollectionView *)collectionView layout:(UICollectionViewLayout *)collectionViewLayout insetForSectionAtIndex:(NSInteger)section
{
    ...
    _itemWidth = (width - flowLayout.minimumInteritemSpacing * (_itemCounts - 1))/_itemCounts;//计算item宽度
    ...
    if (_isSection) {
        ...
        for (NSUInteger j = 0; j < self.innerModelArray.count; j++) {
            ...
            for (NSUInteger i = 0; i < subArray.count; i++) {
                ...
                if (self.renderBlock) {
                    resultCell = self.renderBlock(_collectionViewCell,[NSIndexPath indexPathForRow:j inSection:i],object);
                };
                if (_itemWidth == 0) {
                    UICollectionViewFlowLayout *flowLayout = (UICollectionViewFlowLayout *)_collectionView.collectionViewLayout;
                    CGFloat width = _collectionView.layoutM.width;
                    if (width <= 0) {
