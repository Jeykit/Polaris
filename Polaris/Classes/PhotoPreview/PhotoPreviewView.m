...
@interface PhotoPreviewView()<UIScrollViewDelegate,PSZoomingScrollViewDelegate>
...
@property(strong, nonatomic) PSZoomingScrollView *lastScrollView;
@property(strong, nonatomic) PSZoomingScrollView *currentScrollView;
@property(strong, nonatomic) PSZoomingScrollView *nextScrollView;
@end
...
-(PSZoomingScrollView *)lastScrollView{
    
    if (!_lastScrollView) {
        _lastScrollView = [[PSZoomingScrollView alloc]init];
        _lastScrollView.backgroundColor = [UIColor blackColor];
        _lastScrollView.tapDelegate = self;
    }
    return _lastScrollView;
}
-(PSZoomingScrollView *)nextScrollView{
    if (!_nextScrollView) {
        _nextScrollView = [[PSZoomingScrollView alloc]init];
        _nextScrollView.backgroundColor = [UIColor blackColor];
        _nextScrollView.tapDelegate = self;
    }
    return _nextScrollView;
}
-(PSZoomingScrollView *)currentScrollView{
    if (!_currentScrollView) {
        _currentScrollView = [[PSZoomingScrollView alloc]init];
...
//根据下标设置imgView的image
-(void)setImageView:(PSZoomingScrollView *)imgView withSubscript:(NSInteger)subcript{
